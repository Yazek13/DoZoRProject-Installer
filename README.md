...
Подробно:
1. Команда `sudo usermod -aG docker $USER` добавляет пользователя в группу, но текущая SSH-сессия ещё не применяет это изменение.
2. Вариант A (рекомендуется): выполните `exit`, подключитесь по SSH заново, затем проверьте:
```bash
id -nG
docker ps
```
В выводе `id -nG` должна быть группа `docker`.
3. Вариант B (без переподключения): выполните `newgrp docker`, затем:
```bash
id -nG
docker ps
```
Если `docker ps` всё равно даёт `permission denied`, сделайте Вариант A (полный переподключение SSH).
